...
class Payment {
    ...
    @discardableResult func typeCredentialsInAccountSheet(
        ...
        springboard.secureTextFields.firstMatch.tapWhenHittable(failOnUnmetCondition: false)
    ...
    @discardableResult func handleFreshLoginIfNeeded(
        username: String,
        password: String
    ) -> Self {
        if springboard.alerts.staticTexts["Sign in with Apple ID"].existsAfterWait() {
            typeCredentialsInAccountSheet(username: username, password: password)
